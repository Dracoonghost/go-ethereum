...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"vlry": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + vlry.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + vlry.blockNumber + " (" + new Date(1000 * vlry.getBlock(vlry.blockNumber).timestamp) + ")\n";
		try {
